...
document.addEventListener('DOMContentLoaded', () => {
  // Ensure animations start (keep your existing code)
  document.body.classList.remove('not-loaded');
  ...
  // Attempt passive start (many browsers will block without gesture)
  if (wanted) {
    tryPlay();
  } else {
    updateUI(false);
  ...
  // One-time unlock on first gesture (works across browsers)
  const unlock = async () => {
    if (audio.paused && (wanted || !wanted)) {
      await tryPlay();
    }
    window.removeEventListener('pointerdown', unlock);
    window.removeEventListener('keydown', unlock);
